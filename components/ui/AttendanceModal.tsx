...
import { X, Search, UserCheck, UserX, CheckCircle, XCircle, Users } from "lucide-react";
      ...
      >
        <div className="w-full max-w-lg bg-gradient-to-br from-[#1a2332] to-[#2b3858] rounded-xl shadow-2xl border border-white/20 overflow-hidden">
          ...
          <div className="p-3 space-y-3 max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex gap-2">
              <button onClick={selectAll} className="flex-1 text-xs py-1.5 bg-green-500/20 text-green-300 rounded-lg">
                Все присутствуют
              </button>
            ...
            <div className="space-y-1 max-h-96 overflow-y-auto">
              {filteredStudents.length === 0 ? (
                ...
                filteredStudents.map((student) => {
                  ...
                  const reasonLabel = absenceReasons.find(r => r.id === reason)?.label;

                  return (
                    <div
                      key={student.id}
                      className={`flex items-center gap-2 p-2 rounded-lg transition-colors ${
                        isAbsent ? "bg-red-500/10 border-l-2 border-red-500" : "bg-white/5"
                      }`}
                    >
                      <input
                        type="checkbox"
                      ...
                      <div className="flex-1">
                        <span className={`text-sm text-white ${isAbsent ? "line-through text-gray-400" : ""}`}>
                          {student.name}
                        </span>
                        {isAbsent && reasonLabel && (
                          <span className="text-xs text-orange-400 ml-2">{reasonLabel}</span>
                        )}
                      </div>
                      {isAbsent && (
                        <button
          ...
          <div className="p-3 border-t border-white/10 bg-white/5">
            <button
              ...
              className="w-full py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-lg font-medium disabled:opacity-50 transition-all text-sm"
            >
              {isSubmitting ? "Сохранение..." : "Сохранить отметку"}
            </button>
          </div>
        </div>
      </div>
            ...
            <div className="p-3 space-y-2 max-h-96 overflow-y-auto">
              {absenceReasons.map((reason) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-lg border border-white/10 hover:bg-white/10 transition-colors"
                >
                  <span className="text-2xl">{reason.icon}</span>
                  <div className="flex-1 text-left">
                    <div className={`font-semibold text-sm ${reason.color}`}>{reason.label}</div>
                  </div>
                </button>
              ))}
            </div>
